...
// This function does not seem useful if you can just use Repeat and Modulate.
...
// Note that modulation amount is not an argument.
...
// Amount argument must be large
// This function name is misleading.  The hue is not being modulated, it is doing the modulation.
x = shape(1, 0, 0).color(0, 0, 1)
y = shape(1, 0, 0).color(1, 0, 0)

x.out(o2)
y.out(o3)

shape(3).modulateHue(x, -99).out(o0)
shape(3).modulateHue(y, -99).out(o1)
...
// SetSmooth. Sets audio reactivity smoothness from 0 to 1, using linear interpolation.
// 0 means no smoothing (more jumpy, faster reaction time). 1 means the value will never change.
a.setSmooth(0.8)

// SetCutoff. Sets the lower limit for loudness detection.
a.setCutoff(0.1)

// SetScale. sets the upper limit for  loudness detection, mapping this max volume level to a value of 1
// I think function name is misleading; setCutoffMax would be more accurate
// default is somewhere around 9
a.setScale(8)
